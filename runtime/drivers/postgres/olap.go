...
func (c *connection) Query(ctx context.Context, stmt *drivers.Statement) (*drivers.Result, error) {
	...
	// SQL generated by the runtime (e.g. for metrics views) uses '?' placeholders,
	// but Postgres only supports the '$N' style, so rebind when args are passed.
	// Queries without args (which may legitimately contain '?', e.g. as a JSONB operator) are left untouched.
	query := stmt.Query
	if len(stmt.Args) > 0 {
	...
	if stmt.DryRun {
		_, err = db.ExecContext(ctx, fmt.Sprintf("EXPLAIN %s", query), stmt.Args...)
		return nil, err
	}
	...
	rows, err := db.QueryxContext(ctx, query, stmt.Args...)
	if err != nil {
		if cancel != nil {
			cancel()
		}
...
func (c *connection) QuerySchema(ctx context.Context, query string, args []any) (*runtimev1.StructType, error) {
	res, err := c.Query(ctx, &drivers.Statement{
		Query:            fmt.Sprintf("SELECT * FROM (%s) AS _rill_schema LIMIT 0", query),
		Args:             args,
