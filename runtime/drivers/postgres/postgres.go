...
func (c *ConfigProperties) resolveMaxOpenConns() int {
	if c.MaxOpenConns == 0 {
		// The connection may serve many concurrent dashboard queries when used as an OLAP connector,
		// so the default needs to be larger than 1. Pass a negative value for no limit.
		return 20
	}
...
type connection struct {
	...
	dbRegConnStr string // connection string registered with pgx stdlib, unregistered on Close
}
...
func (c *connection) Close() error {
	...
	if c.dbRegConnStr != "" {
		stdlib.UnregisterConnConfig(c.dbRegConnStr)
	}
	return nil
}
...
func (c *connection) getDB(ctx context.Context) (*sqlx.DB, error) {
	...
	pgxConfig, err := pgx.ParseConfig(c.config.ResolveDSN())
	if err != nil {
		c.dbErr = fmt.Errorf("failed to parse postgres connection string: %w", err)
		return nil, c.dbErr
	}
	// Pin the session timezone to UTC (unless explicitly configured in the DSN)
	// so that SQL generated by the dialect can rely on deterministic casts between TIMESTAMP and TIMESTAMPTZ.
	if _, ok := pgxConfig.RuntimeParams["timezone"]; !ok {
		pgxConfig.RuntimeParams["timezone"] = "UTC"
	}
	c.dbRegConnStr = stdlib.RegisterConnConfig(pgxConfig)

	// Use the versioned driver name: in binaries that also link pgx/v4 (e.g. via the admin service),
	// the unversioned "pgx" name may resolve to v4, which cannot resolve v5's registered conn config.
	c.db, c.dbErr = sqlx.Connect("pgx/v5", c.dbRegConnStr)
	if c.dbErr != nil {
		return nil, c.dbErr
	}
